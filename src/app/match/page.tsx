...
import { ArrowLeft, Lock, Trophy, Award, TrendingUp, X } from "lucide-react";
...
const getTopPerformers = (innings: InningsData | null) => {
  ...
  // 1. Top 3 Batsmen
  const batsmen = Object.keys(battingStats)
    .map((name) => ({
      name,
      ...battingStats[name],
    }))
  ...
  // 2. Top 2 Bowlers
  const bowlers = Object.keys(bowlingStats)
    .map((name) => ({
      name,
      ...bowlingStats[name],
    }))
...
const TeamSummary = ({ teamName, score, wickets, performers }: any) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 mb-4 w-full">
    ...
    <div className="mb-3">
      ...
      {performers?.batsmen && performers.batsmen.length > 0 ? (
        performers.batsmen.map((p: any, i: number) => (
    ...
    <div>
      ...
      {performers?.bowlers && performers.bowlers.length > 0 ? (
        performers.bowlers.map((p: any, i: number) => (
          <div key={i} className="flex justify-between text-sm py-1">
            <span className="text-gray-700 dark:text-gray-300 font-medium">
              {p.name}
            </span>
...
// --- COMPONENT: INNINGS BREAK MODAL ---
const InningsBreakModal = ({ match, onClose }: { match: any, onClose: () => void }) => {
  const perf = getTopPerformers(match.inningsOne);
  const target = match.inningsOne.totalRuns + 1;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl p-6 relative shadow-2xl animate-in zoom-in-95">
        <button 
          onClick={onClose}
        ...
        <div className="text-center mb-6">
          <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Innings Break</h2>
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium mt-1">
            Target: <span className="text-blue-600 dark:text-blue-400 font-bold text-lg">{target}</span>
          </p>
        </div>

        <TeamSummary 
          teamName={match.config.teamOneName}
          score={match.inningsOne.totalRuns}
          wickets={match.inningsOne.wickets}
          performers={perf}
        />

        <button 
          onClick={onClose}
          className="w-full mt-4 py-4 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30"
        >
          Start 2nd Innings
        </button>
      </div>
...
export default function MatchPage() {
  ...
  // Sync view and trigger Innings Break Modal
  useEffect(() => {
    if (match) {
      setViewInnings(match.currentInnings);
      
      // Trigger modal if we just entered 2nd innings and haven't bowled a ball yet
      if (match.currentInnings === 2 && match.inningsTwo?.ballsBowled === 0 && !showInningsBreak) {
         // We use a small timeout or check specifically to ensure it only pops once per session logic
         // For simplicity: If balls is 0, show it. The user closes it to proceed.
  ...
  const isViewingActive =
    viewInnings === match.currentInnings && match.status !== "completed";
  const displayInningsData: InningsData =
    viewInnings === 1 ? match.inningsOne : match.inningsTwo || match.inningsOne;

  // --- PLAYER LIST FILTERING ---
  const isInnings1 = match.currentInnings === 1;
  const battingTeamList = isInnings1
    ? match.config.teamOnePlayers || []
    : match.config.teamTwoPlayers || [];
  const bowlingTeamList = isInnings1
    ? match.config.teamTwoPlayers || []
    : match.config.teamOnePlayers || [];

  const activeBatsmen = [
    displayInningsData.currentStriker,
    displayInningsData.currentNonStriker,
  ];
  
  const battingStats = displayInningsData.battingStats || {};
  const dismissedPlayers = Object.keys(battingStats).filter(
    (player) => battingStats[player]?.outBy
  );

  const availableBatsmen = battingTeamList.filter(
    (p) => !activeBatsmen.includes(p) && !dismissedPlayers.includes(p)
  );

  const availableBowlers = bowlingTeamList.filter(
    (p) => p !== displayInningsData.currentBowler
  );
  ...
  const totalTeamPlayers = battingTeamList.length > 0 ? battingTeamList.length : 10;
  
  const isAllOut =
    displayInningsData.wickets >= totalTeamPlayers ||
  ...
  const isTargetReached =
    ...
    target !== undefined &&
    displayInningsData.totalRuns >= target;

  const isKeypadDisabled = isOversCompleted || isAllOut || isTargetReached;

  // Only show selection modal if active AND keypad is NOT disabled
  if (isViewingActive && !isKeypadDisabled && !showInningsBreak) {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-start text-white p-4 overflow-y-auto">
        <div className="mt-8 mb-6 text-center animate-in zoom-in duration-500">
          <Trophy
            size={64}
            className="text-yellow-400 mx-auto mb-4 drop-shadow-glow"
          />
        ...
        <Link
          href="/"
          className="bg-white text-gray-900 px-8 py-4 rounded-full font-bold text-lg hover:scale-105 transition-transform shadow-xl mb-8"
        >
          Start New Match
        </Link>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 shadow-sm z-10 transition-colors duration-300">
        <div className="p-4 flex items-center justify-between">
          <div className="w-10">
            <Link
              href="/"
              className="p-2 -ml-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full inline-block transition-colors"
            >
              <ArrowLeft size={24} />
            </Link>
          </div>
      ...
      <div className="flex-1 max-w-md mx-auto w-full p-4 flex flex-col">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 text-xs font-bold text-gray-500 uppercase tracking-widest">
          <div className="bg-white dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
            <span className="block mb-1 text-blue-600 dark:text-blue-400">
              Batting
            </span>
            <div className="flex justify-between items-center text-gray-900 dark:text-white text-sm normal-case">
              ...
              <span>
                ...
                {displayInningsData.battingStats?.[
                  displayInningsData.currentStriker || ""
                ]?.runs || 0}
                (
                {displayInningsData.battingStats?.[
          ...
          <div className="bg-white dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
            <span className="block mb-1 text-red-600 dark:text-red-400">
              Bowling
            </span>
            ...
            <div className="text-gray-400 text-xs normal-case mt-1">
              {displayInningsData.bowlingStats?.[
                displayInningsData.currentBowler || ""
              ]?.wickets || 0}
              W -{" "}
              {displayInningsData.bowlingStats?.[
                displayInningsData.currentBowler || ""
        ...
        {isViewingActive ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 bg-gray-100 dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-300 dark:border-gray-700 opacity-70">
            ...
            <p className="text-center text-sm text-gray-500 mt-2 mb-6">
              Viewing past data. <br />
              Switch tabs to resume.
            </p>
